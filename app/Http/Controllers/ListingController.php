<?php

namespace App\Http\Controllers;

use App\Models\Listing;
use App\Models\Tag;
use App\Models\User;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Hash;
use Illuminate\Support\Str;
use ParsedownExtra;

class ListingController extends Controller
{
    public function index(Request $request)
    {
        $listings  = Listing::where('is_active', true)->with('tags')->latest()->get();
        $tags = Tag::orderBy('name')->get();
        // dd($tags);
        if ($request->has('s') && $request->get('s') != null) {
            $query = strtolower($request->get('s'));
            $listings = $listings->filter(
                function ($listing) use ($query) {
                    if (Str::contains(strtolower($listing->title), $query))
                        return true;

                    if (Str::contains(strtolower($listing->company), $query))
                        return true;

                    if (Str::contains(strtolower($listing->location), $query))
                        return true;

                    if (Str::contains(strtolower($listing->content), $query))
                        return true;

                    return false;
                }
            );
        }


        if ($request->has('tag') && $request->get('tag') != null) {
            $tag = strtolower($request->get('tag'));
            $listings = $listings->filter(function ($listing) use ($tag) {
                return $listing->tags->contains('slug', $tag);
            });
        }
        return view('listing.index', compact('listings', 'tags'));
    }



    public function show(Listing $listing)  //Make Sure to name the parameter as the same name in the route (web.php)
    {
        return  view('listing.show' , compact('listing')) ;
    }


    public function apply(Listing $listing , Request $request)
    {
        $listing->clicks()->create(
            [
                'user_agent' => $request->userAgent(),
                'ip' => $request->ip(),
            ]
        );

        return redirect($listing->apply_link);
    }


    public function create()
    {
        return view('listing.create');
    }

    public function store(Request $request)
    {
        //Proccess of Create New listing

        /* 1- Validation */

        $validationArray =
        [
            'title'             => 'required' ,
            'company'           => 'required' ,
            'location'          => 'required' ,
            'content'           => 'required' ,
            'payment_method_id' => 'required',
            'apply_link'        => 'required|url' ,
            'logo'              => 'file|max:2048' ,
        ];

        //If the user not logged in [add user data to the validation]
        if(!Auth::check())
        {
            $validationArray = array_merge($validationArray , [
                'email' => 'email|required|unique:users',
                'name' => 'required',
                'password' => 'required|min:6|confirmed',
            ]);
        }
        $request->validate($validationArray);
        $user = Auth::user();

        if (!$user) {
            $user = User::create(
                [
                    'name' => $request->name,
                    'email' => $request->email,
                    'password' => Hash::make($request->password),
                ]
            );

            $user->createAsStripeCustomer();

            Auth::login($user);
            }

            //payment part

            try
            {
                $amount = 9900;
                if($request->filled('is_highlighted'))
                    $amount += 1900;

                $user->charge($amount , $request->payment_method_id);

                $md = new \ParsedownExtra();
                if($request->has('logo'))
                {
                    $listing = $user->listings()
                    ->create([
                        'title' => $request->title,
                        'slug' => Str::slug($request->title) . '-' . rand(1111, 9999),
                        'company' => $request->company,
                        'logo' => basename($request->file('logo')->store('public')),
                        'location' => $request->location,
                        'apply_link' => $request->apply_link,
                        'content' => $md->text($request->input('content')),
                        'is_highlighted' => $request->filled('is_highlighted'),
                        'is_active' => true
                    ]);
                }
                else
                {
                    $listing = $user->listings()
                    ->create([
                        'title' => $request->title,
                        'slug' => Str::slug($request->title) . '-' . rand(1111, 9999),
                        'company' => $request->company,
                        'location' => $request->location,
                        'apply_link' => $request->apply_link,
                        'content' => $md->text($request->input('content')),
                        'is_highlighted' => $request->filled('is_highlighted'),
                        'is_active' => true
                    ]);
                }

                foreach (explode(',' , $request->tags) as $requestTag) {
                    $tag = Tag::firstOrCreate(
                        [
                            'slug' => Str::slug(trim($requestTag)),
                        ]
                        ,
                        [
                            'name' => ucwords(trim($requestTag))
                        ]
                    );

                    $tag->listings()->attach($listing->id);
                }


                return redirect()->route('listing.index');
            }


            catch (\Exception $e) {
                return redirect()->back()
                ->withErrors(['error' => $e->getMessage()]);
            }
        }
    }
